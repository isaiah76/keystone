...
REFLECTOR_COUNTRY=""
AUR_HELPERS=("paru" "yay" "pamac" "pikaur")
...
set -o pipefail    # pipelines exit with non-zero if any command fails
set -o nounset     # exit when trying to use an undeclared variable
...
last_update(){
  print_step "Checking last system update..."
  local pacman_log="/var/log/pacman.log"

  if [[ ! -f "$pacman_log" ]]; then
    print_info "Last update log file not found. Cannot determine last update time."
    return 0
  fi

  local last_update_time
  last_update_time=$(stat -c %y "$pacman_log" | cut -d '.' -f1)
  local last_update_epoch
  last_update_epoch=$(stat -c %Y "$pacman_log")
  local current_epoch
  current_epoch=$(date +%s)
  local seconds_ago=$((current_epoch - last_update_epoch))
  local days_ago=$((seconds_ago / 86400))
  print_success "Last system update activity detected on: $last_update_time ($days_ago days ago)."
}
...
check_network(){
  ...
  if ping -c 1 -W 2 "archlinux.org" &>/dev/null; then
    ...
    print_error "Network failed. Possibly due to slow internet connection, updates may fail."
...
check_db_lock(){
  ...
  local pid_list=""
  pid_list="$(lsof -t "$lock_file" 2>/dev/null || true)"
  if [[ -z "$pid_list" ]]; then
    pid_list="$(fuser "$lock_file" 2>/dev/null || true)"
    pid_list="$(echo "$pid_list" | tr '\n' ' ' | tr -s ' ')"
  fi

  if [[ -n "$pid_list" ]]; then
    print_error "Pacman DB lock is held by PID(s): $pid_list"
    ...
    if prompt_user "Attempt to kill process(es) $pid_list and remove the lock?"; then
      print_info "Sending TERM signal (15) to PID(s) $pid_list..."
      run_cmd kill -15 $pid_list
      sleep 2
      for pid in $pid_list; do
        if ps -p "$pid" >/dev/null 2>&1; then
          print_info "Process $pid still running. Sending KILL signal (9)."
          run_cmd kill -9 "$pid"
        fi
      done
      run_cmd rm -f "$lock_file"
      print_success "Killed process(es) and removed lock."
    else
...
clean_partial(){
  ...
  local partial_files
  mapfile -t partial_files < <(find /var/cache/pacman/pkg/ -iname "*.part" -type f 2>/dev/null)
...
update_mirrors(){
  ...
  if command -v pacman-mirrors &>/dev/null; then
    ...
    print_info "Ranking the 3 fastest mirrors.."
    ...
    print_info "Neither 'reflector' nor 'pacman-mirrors' found. Skipping mirror update."
  fi
}
...
update_aur(){
  ...
  case "$aur_helper" in
    paru|yay)
      aur_cmd=("$aur_helper" -Sua)    
      ;;
    pikaur)
      aur_cmd=("$aur_helper" -Syu)    
      ;;
    pamac)
      aur_cmd=("pamac" "upgrade" "--aur")  
      ;;
    *)
      print_error "Unsupported helper: $aur_helper"
      return 1
      ;;
  esac
  ...
  print_info "Running AUR update as user '$RUN_AS_USER': ${aur_cmd[*]}"
...
handle_pac_files(){
  ...
  if ! command -v pacdiff &>/dev/null; then
    ...
    local pacfiles
    pacfiles=$(find /etc -type f -name '*.pacnew' 2>/dev/null || true)
    if [[ -n "$pacfiles" ]]; then
  ...
  if [[ -z "$(pacdiff -o 2>/dev/null)" ]]; then
...
check_broken_symlinks(){
  ...
  local broken_links
  mapfile -t broken_links < <(find /etc /usr -xtype l 2>/dev/null || true)
...
check_kernels(){
  ...
  local current_kernel installed_kernels available_kernels eol_kernels
  current_kernel=$(uname -r)
  print_info "Currently running kernel: $current_kernel"

  local kernel_regex='^linux(-lts|-zen|-hardened)?$|^linux[0-9]+$'
  mapfile -t installed_kernels < <(pacman -Qq 2>/dev/null | grep -E "$kernel_regex" || true)
  ...
  mapfile -t available_kernels < <(pacman -Ssq 2>/dev/null | grep -E "$kernel_regex" || true)
  mapfile -t eol_kernels < <(comm -23 <(printf "%s\n" "${installed_kernels[@]}" | sort) <(printf "%s\n" "${available_kernels[@]}" | sort) || true)
...
check_systemd_failed(){
  ...
  failed_units=$(systemctl list-units --failed --no-legend --no-pager 2>/dev/null || true)
...
main(){
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -m|--update-mirrors) UPDATE_MIRRORS="true"; shift ;;
      -c|--country)
        if [[ -z "${2-}" || "${2-}" =~ ^- ]]; then
          print_error "[Error] --country option requires a two letter code." >&2; exit 2
        fi
  ...
  if [[ "$INTERACTIVE" == "true" ]]; then print_info "Running in interactive mode."; fi
  if [[ "$DRY_RUN" == "true" ]]; then print_info "Dry-run mode enabled; no changes will be made."; fi
